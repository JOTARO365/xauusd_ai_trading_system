...
- **Pullback-buy algo** (`agents/algo_registry.py` `PullbackBuyAlgo`) — a **dip-buyer with a tight stop**, built after the CDC guard kept blocking the only live dip-buyer: at gold's current ATR_D1 (~$96) the CDC 2×ATR stop risks ~15.5% of a 41k account per trade, so it can never fire. This one is **long-only** (rides `METALS_LONG_ONLY`), enters a **retracement inside an uptrend** — D1 up (`close_D1 > EMA_D1`) **and** an H1 **reclaim of EMA20 after a dip** (`close[i] > EMA · close[i-1] ≤ EMA`) — and stops just **below the dip's swing low**, capped at `PULLBACK_SL_CAP_ATR`×ATR_H1 (~$8, ≈0.7% risk, so it clears the risk guard). TP = `PULLBACK_RR`×SL, managed (BE + trailing). Validated `scripts/pullback_buy_backtest.py`: XAUUSD **OOS exp_R +0.278, t 3.88**, XAUEUR **+0.138, t 2.05** at RR3 — but **in-sample is weak** (the edge concentrates in the recent gold-bull period, like the momentum book), so it ships **SHADOW-only** (eligible XAUUSD/XAUEUR) to accumulate forward data before going live. Tune via `PULLBACK_EMA`/`PULLBACK_D1_EMA`/`PULLBACK_SWING_LB`/`PULLBACK_SL_CAP_ATR`/`PULLBACK_RR`.
