...
- **Structural SL — stop sits at the D1/H4 candle wick** (`agents/htf_levels.py`, `agents/structural_sl.py`) — fixes "right direction but stopped out first": instead of a fixed/ATR stop in open space, the SL is placed just beyond the **wick-end of the last-closed D1 or H4 candle** (± `STRUCTURAL_SL_BUFFER_ATR`×ATR) — *always*, no clamp, no fallback, even on a small account (lot drops to the minimum to compensate, `standdown`/risk-cap bypassed). `STRUCTURAL_SL_PICK` picks the wick: `farthest` (usually D1, most noise-proof — default) or `nearest` (usually H4, tighter SL). SL only — the original TP is kept (preserving RR pushes TP too far → timeouts). **Backtested** (`scripts/structural_sl_backtest.py`, no look-ahead, 60k H1): **every pair's stop-out rate drops from ~65-73% → ~26-30%**; exp_R improves on GOLD/USDJPY/EURUSD, ~flat on BTC/WTI, and *no* trade ever flips from win→loss. Flags `STRUCTURAL_SL_GOLD` / `STRUCTURAL_SL_MSE`, default OFF. 0 token. A **price-proximity guard** (`ALGO_ENTRY_MIN_GAP_ATR`, default 1.0) skips a new algo entry when a same-direction position is already open within N×ATR — so consecutive momentum bars can't pyramid onto the same spot (a protected/BE position still occupies its combo slot, so it never frees room to re-enter the same point).
...
- **Weekend Gap Monitor & S/R Level Explainer** — dashboard panels: 24/7 gold spot vs Friday close to anticipate the Monday gap, and per-level touch/bounce statistics. Spot source is **XM's own `GOLD24-7#`** (real gold on the same broker, no USDT basis) when it quotes, falling back to crypto gold tokens (PAXG → XAUT → CoinGecko) then AlphaVantage. Override the symbol with `GOLD_SPOT_247_SYMBOL`.
...
- **Pullback-buy algo** — ⚠️ **CUT 2026-08-22 (AUDIT #5).** The claimed `t 3.88` was a statistics bug: `pullback_buy_backtest.py` counted overlapping trades as independent (dedup → `t 1.65`), and the trigger itself carried zero information vs a matched random-long-in-D1-uptrend null (`p 0.570`). A RISK-OFF vol-gate fix still failed drift-null. Removed from `agents/algo_registry.py` (see the CUT log there). Config keys (`PULLBACK_*`) are inert leftovers.
- **CDC Action Zone algo** (`agents/algo_registry.py` `CDCZoneAlgo`, after อ.โฉลก สัมพันธารักษ์) — D1 trend-follow that **holds long / lets winners run**: `EMA2(close)→FastEMA12 / SlowEMA26`, enter with the zone (bull→BUY), **exit only when the zone flips** (Turtle 2×ATR disaster stop). Long-only by default (`CDC_DIR_MODE`; the short leg is −EV). Own paper-resolver (`_resolve_cdc_flip`) + live `_manage_cdc` close-on-flip. Validated `scripts/cdc_backtest.py` + `scripts/cdc_phase2_backtest.py`: with the **pullback entry** (Phase 2 — enter on a dip ≥`CDC_PULLBACK_MIN` from the recent high, after อ.โฉลก's wave-2/4 "buy the retracement"; an RSI filter was tested and *hurt*, so it's excluded), XAUUSD reaches exp_R **+0.98, t 2.05, OOS +2.48** — t clears the significance bar and beats `tsmom_d1` ~6×, but n≈47 (position-style = few trades) is still under the n≥80/100 count bar, so it stays **SHADOW** until forward data accumulates. Eligible: XAUUSD / XAUEUR / BTCUSD. **Turtle pyramiding** (`CDC_PYRAMID`, default **off**) adds up to `CDC_MAX_UNITS` units every `CDC_ADD_HALF_N`×ATR in favour for the "big order, ride the trend" behaviour (backtest: ~3–4× the single-unit sum-R); it's money-management-sensitive, so it stays off until the algo is live and reviewed.
...
**`NEWS_GATE` retired (2026-08-22, T-06):** `NEWS_GATE` and `NEWS_CONTRA_ENABLED` flags have
been removed. They lived inside `decision_maker._run_gates`, which is entirely skipped under
`REGIME_LIVE=true` — making the flags dead code on the live path. The opposition-block intent
is deferred to T-17 as a shadow-only veto in `agents/entry_gate.py` (not yet implemented).
`macro_quant.gold_macro_score` is now explicitly **display-only** (dashboard `/api/macro-quant`);
the single live consumable sentiment number is `sentiment_score.get_score().score` (ARCHITECTURE §5).
...
- **Event Reaction Stats table** — gold's 15-year historical reaction to every tracked US release, sorted by impact: N, up%/down%, avg move %, the **≈ $ move at the live price** (`avg move% × current bid`), ×-normal-day, and D+2 follow-through. Built by `scripts/gen_extra_event_dates.py` → `scripts/event_reaction_stats.py` (`data/event_stats.json`), now covering **13 events** (added `JOBLESS_CLAIMS` weekly, `PPI`, `RETAIL_SALES`, `CB_CONF`, `GDP` alongside NFP/CPI/CORE_CPI/FOMC/UNEMPLOYMENT/ISM_MFG/ISM_SVC/ADP). Table renders dynamically from the data — add an event's dates and it appears. 0 token.
...
- **Structural Read (โค้ช)** — a real-time plain-language read of market structure (after the ทองคำคนวงใน / UHAS analyst's on-chart coaching), computed in the browser from the same S/R ladder + live price + trend: **position-in-range %**, nearest support/resistance with **strength score + touch count + "เตะเป็นจริง"** (bounce-confirmed) flag, an **action-state** (ทดสอบต้าน·รอเบรก / ทดสอบรับ·รอเด้ง / กลางกรอบ), a **probabilistic path** ("likely test X · bounce N% / break M%" from the level's bounce/break stats), and the **structure-change level** whose break flips the short-term trend. Updates with the 5 s price / 5 min zone refresh. 0 token.
...
does *not* survive out-of-sample (window bias), and sweep-fade is a high-WR/low-RR trap. Both were
kept shadow-only for a while and then ⚠️ **CUT 2026-08-22 (AUDIT #5)**: `regime_momentum_fvg` (FVG
filter value ≈ 0 over a base already NULL in AUDIT #4) and `sweep_reversal` (−0.056 t−1.91, BUY-only
fix worse at cost×2). Both removed from `agents/algo_registry.py` (see the CUT log there);
`scripts/smc_backtest.py` remains for the historical record. `setup.py` step 7 does this
...
| `OPEN_ORDER_FINE_LOCK` | `false` | **B11 fix.** When ON, `open_order` releases the `_mt5_lock` during its retry-backoff `sleep` (fine-grained locking — only the MT5 setup/send regions stay locked), so the Guardian thread isn't starved for ~1s while an order retries. `false` = the whole `open_order` runs under one lock (old behavior; sleep is under the lock). Enable it together with `GUARDIAN_ENABLED`, otherwise the Guardian has nothing to be starved from. |
| `CYCLE_DEADLINE_SEC` | `0` | **B9 fix.** Enforceable per-cycle time ceiling. When `>0`, `run_cycle` wraps `TRADING_APP.ainvoke` in `asyncio.wait_for`; on timeout (a stalled LLM call — worst case is 4 calls × 40s × 2 ≈ 320s, longer than the 300s interval) it cancels the graph and runs a **protective fallback** (`ensure_sl_protection` → momentum-exit → zone-break → breakeven → trailing) so open positions are managed even though `position_mgmt` normally sits downstream of the LLM chain. `0` = disabled (old behavior: no ceiling, only a post-hoc `[CYCLE_TIME]` warning). |
...
| `HTF_FADE_BLOCK` | `true` | No SELL at D1/W1 SUPPORT / no BUY at RESISTANCE (gold LLM path) |
| `SR_ENTRY_GATE` | `true` | **Algo S/R gate** — blocks BUY into a strong near resistance / SELL into strong near support (swing-pivot ladder from bars, momentum-aware: a broken level lets price through). Applies in `multi_symbol_executor` to **every pair EXCEPT breakout algos** (`SR_BREAKOUT_ALGOS`) — breakout strategies must be free to trade through levels. Block-only — never adds a trade. See `agents/sr_entry_gate.py`; tune via `SR_BLOCK_ATR`/`SR_MIN_TOUCHES`/`SR_LOOKBACK`/`SR_PIVOT`/`SR_CLUSTER_ATR`. Note: fade/reversal algos already trade the correct side of a level, so the gate mostly catches a fader entering straight into an opposing wall. `scripts/sr_gate_backtest.py` + `data/sr_gate_combos.json` remain as the per-combo evidence record. |
| `SR_BREAKOUT_ALGOS` | `regime_momentum,...,tsmom_d1` | Breakout-exempt list — **legacy mode only** (used when `SR_GATE_COMBOS` is empty). When an allowlist is set, this has no effect. |
| `SR_GATE_COMBOS` | `` (empty) | **Allowlist mode** for the S/R gate — comma list of `algo\|SYMBOL` combos to gate; nothing else is gated. Set → the gate applies to exactly these combos (breakout tag ignored); empty → fall back to `data/sr_gate_combos.json` (backtest-passing combos), then to legacy (gate-all-except-breakout). Also wired into the **gold entry paths outside MSE**: `regime_tick` (regime_momentum) and `tsmom_manager` (tsmom_d1) now call `sr_entry_gate.blocks_live_gold(...)` before `open_order`, so a gold BUY into resistance / SELL into support is blocked there too (not just in MSE). Live set: `macro_momentum\|XAUUSD` (backtest-passed) + `regime_momentum\|XAUUSD`, `tsmom_d1\|XAUUSD` (risk-shape: Δexp_R≥0 but t<2 — kept for RR/drawdown protection, not a proven edge). `pullback_buy` deliberately excluded (gate hurts it). |
| `SR_GATE_ALL` | `false` | **Every algo looks at the zone before acting** — when true the gate applies to all combos (block-only, momentum-aware), above the allowlist. Recommended together with `SR_RICH_ZONE` so it only blocks high-conviction zones. |
| `SR_GATE_EXCLUDE` | `pullback_buy` | Combos/algos always exempt from the gate (comma `algo` or `algo\|SYMBOL`). `pullback_buy` is excluded because the backtest shows the gate blocks its profitable dips (Δ−0.022). |
| `SR_RICH_ZONE` / `SR_RICH_MIN_BOUNCE` / `SR_RICH_MIN_TESTS` | `false` / `55` / `3` | **Rich-zone block** — the gate blocks only zones with a causal, statistically-significant bounce history (bounce_pct ≥ min over ≥ min tests, no look-ahead), not merely a nearby swing. Very selective (rarely fires), which makes `SR_GATE_ALL` safe. Validate with `scripts/sr_gate_backtest_rich.py` → `data/sr_gate_combos_rich.json`. Fundamentals (fair-value / option-wall / COT) stay display-only — no causal per-bar history to backtest. |
| `ALGO_ROUTER_ENABLE` / `ALGO_ROUTER_LIVE` / `ALGO_ROUTER_ALLOW_PROMOTE` | `false` / `false` / `false` | **LLM algo-router** — periodically (or on a loss-streak) an LLM suggests which combos suit the current regime. `ENABLE` runs the LLM (costs tokens); `LIVE` lets it write switch state; `ALLOW_PROMOTE` gates **auto-promotion to LIVE**. Roster-drift guard: with `ALLOW_PROMOTE=false` (default) the LLM can only **demote** (→ SHADOW); promotions to LIVE are blocked and logged as suggestions, because promoting real money onto a thin/negative-edge combo is a validation/human decision, not an LLM whim. `loss_adaptive` still auto-demotes via its own flag. |
| `SL_MIN_PIPS` / `SL_BACKSTOP_PIPS` | `100` / `500` | **SL-enforce** (`agents/sl_enforce.py`) — refuse-to-open safety floor: `open_order`/`_order_setup` reject any entry whose `sl_pips` is missing/0/below `SL_MIN_PIPS`, so a stopless order (the −6,248 un-stopped-SELL class) can never be placed. `SL_BACKSTOP_PIPS` is the disaster backstop distance (the existing `ensure_sl_protection` sweep already attaches a backstop SL to any stopless open position across **all** symbols/magics every cycle). Floor is a hard safety net, not a change to SL/TP *defaults*. |
| roster-guard (`agents/roster_guard.py`, no flag) | — | **LIVE allowlist assertion** — a code-side frozen `LIVE_ALLOWLIST` (not dashboard/LLM-editable); `assert_roster()` runs every cycle and **force-demotes any LIVE combo not on the allowlist** → SHADOW (demote-only, never promotes). Closes every roster-drift door (LLM router, human toggle, engine seed) — e.g. `regime_momentum_fvg:XAUUSD` was LIVE against its own SHADOW-ONLY docstring. Change the allowlist = edit the module (owner action). Current allowlist: `regime_momentum:XAUUSD`, `macro_momentum:XAUUSD`, `tsmom_d1:BTCUSD`, `tsmom_d1:WTIUSD`. |
| `TREND_BLOCK_NEUTRAL` / `TREND_BLOCK_NEUTRAL_SHADOW` | `false` / `true` | **Gold-fit block-neutral filter** (`agents/gold_regime_filter.py`) — blocks gold momentum-breakout entries when the **D1 drift is NEUTRAL/choppy** (the slice that loses robustly: −0.199R, t−2.62, negative every quartile on 25y). Wired into `regime_tick` + `regime_executor` (block-only, causal EMA480+slope on closed H1 bars, parity with `scripts/trend_filter_backtest.py`). Combined with `METALS_LONG_ONLY` (long-bias) this is the "tune to gold" config: it lifts gold momentum from −0.054R to +0.047R over 25y. ⚠️ **This is drift-harvesting (beta), not alpha** — t0.93 (not significant), 2/4 quartiles, works only while gold trends up. `SHADOW=true` logs a would-block without acting; set `false` to enforce live. Kill = `TREND_BLOCK_NEUTRAL=false`. |
| `MACRO_STRIP_AUTO_REFRESH` | `true` | **Macro-strip auto-refresh** — the main loop refreshes `data/macro_strip.json` (DXY / 10Y yield / real-yield, shown on the dashboard + read by `sentiment_score`/`weekly_outlook`) once per UTC day via a background thread running `scripts/fetch_macro_strip.py` (3 AlphaVantage requests, inside the free 25/day quota, fail-soft — old data kept on quota/network error). Fixes the stale-macro-strip gap (the file previously only updated when the script was run by hand/cron, so it drifted ~a month out of date). Display/context only — never an entry signal (AUDIT #6 proved DXY-as-gate has no edge). Kill = `false`. |
| `CONFIRM_GATE` | `false` | **Candle-confirmation gate** — filters an algo's own fakeout/rejection signals before entry (price-action confirm). Per-algo mode (`agents/confirm_gate.py` `MODE_BY_ALGO`): momentum/trend = `cont` (signal bar must **close strong** in the trade direction), fade = `rev` (signal bar must be a **reversal pin/hammer** at the extreme). Block-only — never adds a trade. Backtest evidence: `scripts/confirm_gate_backtest.py` (same-TF) + `scripts/confirm_tf_matrix.py` (which timeframe's candle-close confirms each algo best → `docs/reviews/confirm-tf-matrix.md`). Key result: **momentum algos improve most when confirmed by a higher-TF close (D1 best)**; mean-reversion is hurt by any confirm (leave OFF). Default OFF — no combo cleared the strict live bar yet. |
| `CONFIRM_CLV` | `0.5` | Confirm threshold — the signal candle's close must sit past this fraction of its range toward the trade side (0.5 = past the mid). |
| `LONG_ONLY_ALL` | `false` | **Global long-only kill-switch** — blocks every SELL on **every** symbol (a superset of `METALS_LONG_ONLY`), at the `open_order` choke point **and** the pending-order path (`SELL_LIMIT` fades/hedges), live orders only (shadow SELLs still log). Backed by three independent cuts of real closed-trade data: DB by-direction **BUY +86k / SELL −79k**, account `381706956` **BUY +2,417 / SELL −2,440** over 324 trades, and the per-algo Real Edge (every short combo is the loss leader). ⚠️ This is a **regime bet** (the sample is the 2024–25 gold bull); flip it to `false` the moment the market turns bearish, since shorts work in downtrends. |
| `METALS_LONG_ONLY` | `false` | **Kill-switch for metal shorts** — blocks every SELL on a precious-metal symbol (`XAU*`, `XAG*`, anything containing `GOLD`/`SILVER`) at the `open_order` choke point, so it covers all order paths (decision, pending, zone-reentry, MSE) at once. Live orders only — `shadow` (paper) SELLs still log for counterfactual data. Added after a demo drawdown where shorting the gold/silver bull with wide structural SLs lost ~9.4k; metals have no validated short edge. BTC and other non-metal shorts are unaffected. |
...
| `EOD_PROFIT_CLOSE_HOUR_BKK` | 2 | Small-capital no-overnight-carry: while equity < `FORCE_CLOSE_MIN_CAPITAL`, **during this exact hour** (BKK/UTC+7 — `2` = 02:00–02:59, i.e. 2 AM Thai, **not** 2 PM), the first cycle where the **basket floating PnL (all system positions summed, incl. swap) is > 0** force-closes **every system position** — flat, locking the net gain — once per day. While the basket is ≤ 0 it waits (positions ride their SLs); if never net-positive during that hour it skips the day. `-1` disables. |
