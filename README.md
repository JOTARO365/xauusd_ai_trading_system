...
| `SR_ENTRY_GATE` | `true` | **Algo S/R gate** — blocks BUY into a strong near resistance / SELL into strong near support (swing-pivot ladder from bars, momentum-aware: a broken level lets price through). Applies in `multi_symbol_executor` to combos in `data/sr_gate_combos.json` only (those that passed `scripts/sr_gate_backtest.py`: gated `exp_R>0·OOS≥0·t≥2·n≥80·not worse·keeps ≥15% of trades`). Block-only — never adds a trade. See `agents/sr_entry_gate.py`; tune via `SR_BLOCK_ATR`/`SR_MIN_TOUCHES`/`SR_LOOKBACK`/`SR_PIVOT`/`SR_CLUSTER_ATR`. Re-run the eval to refresh the allowlist. |
