...
- **Structural SL — stop leans on D1/W1 S/R** (`agents/htf_levels.py`, `agents/structural_sl.py`) — fixes "right direction but stopped out first": instead of a fixed/ATR stop sitting in open space, the SL is placed just beyond the nearest **D1/W1 swing level** (± `STRUCTURAL_SL_BUFFER_ATR`×ATR), clamped to `[STRUCTURAL_SL_MIN_ATR, STRUCTURAL_SL_MAX_ATR]`×ATR — outside that band it falls back to the original SL (opt-in per trade). TP recomputes to preserve the original RR. **Backtested before shipping** (`scripts/structural_sl_backtest.py`, no look-ahead): **gold ALGO-mom +0.03~0.04R robust across every buffer → enable-able (`STRUCTURAL_SL_GOLD`)**; **WTI/MSE momentum got *worse* (−0.02~0.035R) because a wider stop breaks the validated tight-SL×0.7 edge → keep `STRUCTURAL_SL_MSE=false`**. Both flags default OFF. 0 token.
