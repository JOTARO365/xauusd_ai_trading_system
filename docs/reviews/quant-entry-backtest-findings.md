...
## Synthetic null-test (`scripts/gold_sim_nulltest.py`) — 2026-07-18 (h): "edge" = harness drift-bias
ใช้ synthetic data ถูกทาง (เกมรับ/null-check ไม่ใช่หา edge): block-bootstrap gold (สลับบล็อก 16 m15 →
ทำลาย S/R+trend, เก็บ return distribution + per-bar OHLC geometry) → รัน **sim ตัวจริง 100%** 8 ชุด →
null distribution ของ avg net-R/ไม้ @30pt.

**ผล:** null mean **+0.027** (std 0.055, range −0.063..+0.119) vs **REAL +0.030** → z=+0.04, **p(null≥real)=0.62**.
→ **"+0.030R edge" ไม่ใช่ signal** — harness ทำเงินเท่ากันบน data ที่ทำลาย structure ทิ้งหมด = **drift-capture**
(uptrend ทอง 2020-24 + h4_bias มัก BULLISH → BUY เยอะ → กิน drift แม้สลับบล็อก) ไม่ใช่ S/R skill.
DSR/PBO/OOS บอกแค่ "marginal/อ่อนลง" — null-test **ฟันธงเชิงประจักษ์ว่าเป็น bias**. gold structure algo ปิดเด็ดขาด.
