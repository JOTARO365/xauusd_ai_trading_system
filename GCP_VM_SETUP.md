...
## ขั้นตอนที่ 1 — สร้าง Project + เปิด Billing
...
1. แก้ `scripts/setup_vm.ps1` 2 บรรทัด (หรือเตรียมค่าก่อนวาง) ให้ path ตรงกับที่ auto-deploy ใช้:
   ```powershell
   $GH_TOKEN = "ghp_xxxxxxxx"   # GitHub PAT
   $DEST     = "C:\"            # ← clone ลง C:\xauusd_ai_trading_system (auto-deploy คาดหวัง path นี้)
   ```
...
3. โหลด script (ถ้ายังไม่มี repo) — clone ด้วยมือก่อน หรือวางเนื้อหา `setup_vm.ps1` ทั้งไฟล์:
   ```powershell
   git clone https://ghp_xxxx@github.com/JOTARO365/xauusd_ai_trading_system.git C:\xauusd_ai_trading_system
   cd C:\xauusd_ai_trading_system
...
```powershell
notepad C:\xauusd_ai_trading_system\.env
```
...
| auto-deploy ไม่ pull | path ไม่ตรง → auto_deploy.ps1 คาดหวัง `C:\xauusd_ai_trading_system` |
