...
def _run_gates(chart_data: dict, sentiment_data: dict, advisor_data: dict | None,
    ...
    # 7+8. MOMENTUM_BREAKOUT fast path — ข้าม SR zone gate และ ATR gate
    hour_now    = datetime.now(_tz.utc).hour
    is_london_ny = 13 <= hour_now < 17
    is_mom_fast  = entry_type == "MOMENTUM_BREAKOUT" and (
        conf >= 70 or (is_london_ny and conf >= 68)
    )
    if is_mom_fast:
        logger.info(
            f"MOMENTUM_BREAKOUT fast path: skip SR+ATR gates "
            f"(conf={conf}%, {'London/NY' if is_london_ny else 'other session'})"
        )

    # 7. No SR zone — ต้อง conf สูงกว่า (ข้ามถ้า momentum fast path)
    if not is_mom_fast and sr_zone == "NONE" and conf < 62:
        return _fail(f"No SR zone requires conf ≥62% (got {conf}%)")

    # 8. ATR gate — ตลาดผันผวน (ข้ามถ้า momentum fast path)
    h4_atr = chart_data.get("indicators", {}).get("h4", {}).get("atr", 0)
    if not is_mom_fast and h4_atr > 20 and conf < 62:
    ...
    # 11. Losing streak — gradual position reduction (ไม่ block แต่ลด size)
    streak_scale = 1.0
    if _cfg.STREAK_PROTECTION:
        ...
        if streak >= 5:
            ...
            streak_scale = 0.80
        if streak_scale < 1.0:
            logger.info(f"Streak {streak}: position size ×{streak_scale} (gradual reduction)")
...
Account — Today: {history['today_pnl']:+.2f} USD ({history['today_trades']} trades) | WR10: {history['last_10_winrate']}% | Streak: {history['losing_streak']}L"""
    ...
    if decision == "EXECUTE" and out_direction in ("BUY", "SELL"):
        ...
        # ── Confidence-based position sizing + streak reduction ──
        ...
        if streak_scale < 1.0:
            conf_scale = max(_conf_min_s * streak_scale, conf_scale * streak_scale)
            logger.info(
                f"Position sizing: conf={conf}% scale={conf_scale / streak_scale:.2f} "
                f"× streak_scale={streak_scale} → final={conf_scale:.2f}"
            )
        else:
