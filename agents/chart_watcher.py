...
# ─────────────────────────────────────────────────────────────
#  DISPLAY-ONLY (0-token) — UHAS terminal features (คลิป 1jprxG_Tq5M)
# ─────────────────────────────────────────────────────────────

def detect_fvg(df, lookback=40, max_out=4, point=0.01):
    """A: Fair Value Gap (SMC 3-candle imbalance). bull: high[i-1] < low[i+1] (gap up);
    bear: low[i-1] > high[i+1]. เก็บเฉพาะ gap ที่ 'ยังไม่ถูก fill' (ราคายังไม่ย้อนเข้ากรอบ). Display-only."""
    if df is None or len(df) < 3:
        return []
    r = df.tail(lookback).reset_index(drop=True)
    hi, lo = r["high"].values, r["low"].values
    current = float(r["close"].iloc[-1])
    out = []
    for i in range(1, len(r) - 1):
        if hi[i - 1] < lo[i + 1]:
            typ, top, bot = "bull", float(lo[i + 1]), float(hi[i - 1])
        elif lo[i - 1] > hi[i + 1]:
            typ, top, bot = "bear", float(lo[i - 1]), float(hi[i + 1])
        else:
            continue
        # filled = ราคาหลังเกิด gap เคยย้อนเข้ากรอบ [bot,top]
        filled = any(lo[k] <= top and hi[k] >= bot for k in range(i + 2, len(r)))
        if filled:
            continue
        mid = round((top + bot) / 2, 2)
        out.append({"type": typ, "top": round(top, 2), "bottom": round(bot, 2),
                    "mid": mid, "size_pips": round((top - bot) / point),
                    "dist_pct": round((mid - current) / current * 100, 2)})
    out.sort(key=lambda g: abs(g["dist_pct"]))
    return out[:max_out]


def detect_double_pattern(df, window=3, lookback=80, eq_pct=0.0015):
    """C: double top/bottom จาก swing pivots + measured-move target. Display-only. คืน dict หรือ None."""
    ...
    for i in range(window, len(r) - window):
        if all(hv[i] >= hv[i - j] for j in range(1, window + 1)) and \
           all(hv[i] >= hv[i + j] for j in range(1, window + 1)):
            sh.append((i, float(hv[i])))
        if all(lv[i] <= lv[i - j] for j in range(1, window + 1)) and \
           all(lv[i] <= lv[i + j] for j in range(1, window + 1)):
            sl.append((i, float(lv[i])))
    if len(sh) >= 2 and abs(sh[-1][1] - sh[-2][1]) <= sh[-1][1] * eq_pct:
        troughs = [p for idx, p in sl if sh[-2][0] < idx < sh[-1][0]]
        if troughs:
            neck, top = min(troughs), (sh[-1][1] + sh[-2][1]) / 2
            return {"pattern": "DOUBLE_TOP", "direction": "SELL", "level": round(top, 2),
                    "neckline": round(neck, 2), "target": round(neck - (top - neck), 2)}
    if len(sl) >= 2 and abs(sl[-1][1] - sl[-2][1]) <= sl[-1][1] * eq_pct:
        peaks = [p for idx, p in sh if sl[-2][0] < idx < sl[-1][0]]
        if peaks:
            neck, bot = max(peaks), (sl[-1][1] + sl[-2][1]) / 2
            return {"pattern": "DOUBLE_BOTTOM", "direction": "BUY", "level": round(bot, 2),
                    "neckline": round(neck, 2), "target": round(neck + (neck - bot), 2)}
    return None
...
def _build_sr_meta(h4_sr: dict, h1_sr: dict, key_lvl: dict,
    ...
    for _m in meta:                                                      # B UHAS: unified 0-100 score/grade
        _m["score"], _m["grade"] = _score_zone(_m)
    return meta
...
ENTRY_TYPE: SR_ZONE หรือ ENGULFING หรือ STRUCTURE_PULLBACK หรือ BREAKOUT_RETEST หรือ EMA_PULLBACK หรือ MOMENTUM_BREAKOUT หรือ DOJI_AT_ZONE หรือ NONE"""
    ...
    result = {
        ...
        "fvg":             detect_fvg(m15.get("df")),                              # A UHAS (display-only)
        "double_pattern":  detect_double_pattern(h1.get("df")),                    # C UHAS (display-only)
        "key_levels":    key_lvl,
